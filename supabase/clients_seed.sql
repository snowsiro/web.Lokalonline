...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM clients WHERE email = 'yori2wien@gmail.com') THEN
  ...
  ELSE
    UPDATE clients SET
      name             = 'Mija Chun',
      business_name    = 'Yori2 Korean Restaurant',
      phone            = '+43 57 333 789',
      plan             = 'flat',
  ...
  IF NOT EXISTS (SELECT 1 FROM clients WHERE email = 'seoulmate.vienna@gmail.com') THEN
    INSERT INTO clients (name, business_name, email, phone, plan, billing_cycle, setup_fee_waived, start_date, next_billing, page_url, status, notes)
    VALUES (
      'Yena Lee',
  ...
  ELSE
    UPDATE clients SET
      name             = 'Yena Lee',
      business_name    = 'Seoulmate Korean Bistro',
      phone            = '+43 676 6537332',
      plan             = 'flat',
      billing_cycle    = 'monthly',
      page_url         = 'seoulmate',
      status           = 'active',
  ...
  IF NOT EXISTS (SELECT 1 FROM orders WHERE email = 'yori2wien@gmail.com') THEN
  ...
  ELSE
    UPDATE orders SET
      site_slug         = 'yori2',
      legal_type        = 'gesellschaft',
      legal_name        = 'Tonguc Gesellschaft m.b.H Nfg KG',
      legal_form        = 'KG',
      geschaeftsfuehrer = 'Mija Chun',
      fn_number         = '196843 a',
      uid_number        = 'ATU 60869411',
      payment_status    = 'paid'
    WHERE email = 'yori2wien@gmail.com';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM orders WHERE email = 'seoulmate.vienna@gmail.com') THEN
    INSERT INTO orders (business_name, business_type, email, phone, address, contact_name, site_slug, legal_type, legal_name, legal_form, geschaeftsfuehrer, fn_number, uid_number, payment_status)
    VALUES (
      'Seoulmate Korean Bistro',
      'Schnellimbiss',
